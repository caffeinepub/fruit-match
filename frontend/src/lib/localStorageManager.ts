...
function createDefaultPlayerData(username: string, theme: string, language: string): LocalPlayerData {
...
export function loadPlayerData(): LocalPlayerData | null {
  try {
    ...
    console.log('[LocalStorage] ✓ Successfully loaded player data for:', parsed.username);
    return parsed;
  } catch (error) {
...
export function savePlayerData(data: LocalPlayerData): void {
  try {
    ...
    console.log('[LocalStorage] ✓ Successfully saved player data for:', data.username);
  } catch (error) {
...
export function initializePlayer(username: string, theme: string, language: string): LocalPlayerData {
...
export function updateLevelProgress(worldId: number, levelId: number, stars: number): void {
  const data = loadPlayerData();
  if (!data) {
    console.warn('[LocalStorage] Cannot update level progress: No player data found');
    return;
  }
  ...
  if (!level) {
    console.warn(`[LocalStorage] Level ${levelId} not found in world ${worldId}`);
    return;
  }
  ...
  savePlayerData(data);
  console.log(`[LocalStorage] ✓ Updated level ${worldId}-${levelId} with ${stars} stars. Total stars: ${data.totalStars}`);
}
...
function checkAndUnlockWorlds(data: LocalPlayerData): void {
  for (let i = 1; i < data.worlds.length; i++) {
    ...
    const previousWorldStars = previousWorld.levels.reduce((sum, l) => sum + l.stars, 0);
...
export function consumePowerUp(type: keyof PowerUpCounts): boolean {
  const data = loadPlayerData();
  if (!data) {
    console.warn('[LocalStorage] Cannot consume power-up: No player data found');
    return false;
  }
  ...
  savePlayerData(data);
  console.log(`[LocalStorage] ✓ Consumed ${type} power-up. Remaining: ${data.powerUpCounts[type]}`);
  return true;
}
...
export function getDailyRewardStreak(): { currentStreak: number; isAvailable: boolean; lastClaimDate: number } {
  const data = loadPlayerData();
  if (!data) {
...
export function claimDailyReward(): { newStreak: number; rewards: PowerUpCounts } {
  const data = loadPlayerData();
  if (!data) {
  ...
  const now = Date.now();
  const lastClaim = data.dailyRewardData.lastClaimDate;
  const hoursSinceLastClaim = lastClaim === 0 ? 999 : (now - lastClaim) / (1000 * 60 * 60);
  ...
  if (hoursSinceLastClaim > 48 || lastClaim === 0) {
    ...
    if (data.dailyRewardData.streakHistory.length > 7) {
      data.dailyRewardData.streakHistory = data.dailyRewardData.streakHistory.slice(-7);
    }
  } else {
  ...
  Object.entries(rewards).forEach(([type, count]) => {
    if (count && count > 0) {
      data.powerUpCounts[type as keyof PowerUpCounts] += count;
    }
  });
  ...
  savePlayerData(data);
  console.log(`[LocalStorage] ✓ Claimed daily reward for day ${newStreak}, power-ups applied atomically`);
...
export function claimLevelChest(chestId: number): void {
  const data = loadPlayerData();
  if (!data) {
    console.warn('[LocalStorage] Cannot claim level chest: No player data found');
    return;
  }
...
export function resetPlayerData(): void {
  ...
  console.log('[LocalStorage] ⚠️ Player data has been reset by explicit user action');
}
