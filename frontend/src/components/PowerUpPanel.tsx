...
import { PowerUp } from '@/lib/gameLogic';
...
export default function PowerUpPanel({ powerUps, onActivate, themeColor }: PowerUpPanelProps) {
  ...
  const displayPowerUps = safePowerUps.length > 0 ? safePowerUps : DEFAULT_POWER_UPS;

  // Additional safety check: ensure we have at least some power-ups to display
  if (!displayPowerUps || displayPowerUps.length === 0) {
    console.warn('[PowerUpPanel] No valid power-ups to display, using fallback');
    return (
      <div className="powerup-panel-compact">
        <div className="flex items-center justify-center gap-2">
          {DEFAULT_POWER_UPS.map((powerUp) => {
            const config = POWER_UP_CONFIG[powerUp.type as keyof typeof POWER_UP_CONFIG];
            return (
              <div
                key={powerUp.type}
      ...
      <TooltipProvider>
        <div className="flex items-center justify-center gap-2">
          {displayPowerUps.map((powerUp) => {
            ...
            if (!config) {
              console.warn(`[PowerUpPanel] No config found for power-up type: ${powerUp.type}`);
              return null;
            }
            ...
            return (
              <Tooltip key={powerUp.type}>
                    ...
                    <div className="relative">
                      <img
                        ...
                        className={`w-8 h-8 object-contain transition-all ${
                          isDisabled ? 'grayscale' : 'hover:scale-110 drop-shadow-md'
                        }`}
                        onError={(e) => {
                          console.error(`[PowerUpPanel] Failed to load icon: ${config.icon}`);
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 mt-1">Kalan: {powerUp.count}</p>
                </TooltipContent>
              </Tooltip>
            );
          })}
