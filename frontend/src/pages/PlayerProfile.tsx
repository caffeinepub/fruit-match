...
import { ArrowLeft, User, Star, Trophy, Palette, Volume2, VolumeX, ExternalLink, RotateCcw, Edit2, Check, X, Bomb, Clock, Lightbulb, Shuffle } from 'lucide-react';
...
const THEME_GRADIENTS: Record<Theme, string> = {
  [Theme.garden]: 'from-fruit-garden-light/80 via-fruit-garden/60 to-fruit-garden-dark/80',
  [Theme.ocean]: 'from-fruit-ocean-light/80 via-fruit-ocean/60 to-fruit-ocean-dark/80',
  [Theme.candyland]: 'from-fruit-candyland-light/80 via-fruit-candyland/60 to-fruit-candyland-dark/80',
  [Theme.forest]: 'from-fruit-forest-light/80 via-fruit-forest/60 to-fruit-forest-dark/80',
  [Theme.volcano]: 'from-fruit-volcano-light/80 via-fruit-volcano/60 to-fruit-volcano-dark/80',
  [Theme.space]: 'from-fruit-space-light/80 via-fruit-space/60 to-fruit-space-dark/80',
};

export default function PlayerProfile({ onBackToTitle, currentTheme }: PlayerProfileProps) {
  const [playerData, setPlayerData] = useState<LocalStorage.LocalPlayerData | null>(null);
  ...
  const totalLevelsCompleted = playerData
    ? playerData.worlds.reduce((sum, world) => 
        sum + world.levels.filter(level => level.completed).length, 0
      )
    : 0;

  const powerUpCounts = playerData?.powerUpCounts || { bomb: 0, clock: 0, shuffle: 0, magnifier: 0 };
  ...
  const handleReset = () => {
    console.log('[PlayerProfile] ⚠️ User confirmed data reset - clearing all localStorage data');
    LocalStorage.resetPlayerData();
    console.log('[PlayerProfile] ✓ Data reset complete - returning to title screen');
    onBackToTitle();
  };
  ...
  if (!playerData) {
    return (
      <div className="relative min-h-screen w-full flex items-center justify-center">
        <div className={`fixed inset-0 bg-gradient-to-br ${themeGradient} pointer-events-none transition-all duration-1000`} />
        <div className="relative z-10 text-white text-2xl font-bold">{t('loading')}</div>
      </div>
    );
  }
            ...
            <AlertDialogContent className="bg-white/95 backdrop-blur-md">
              <AlertDialogHeader>
                ...
                <AlertDialogDescription className="text-lg text-gray-600">
                  Oyun verilerinizi sıfırlamak istediğinizden emin misiniz? Bu işlem geri alınamaz.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <Card className="bg-white/95 backdrop-blur-md shadow-xl hover:shadow-2xl transition-shadow">
            ...
            <CardContent>
              ...
              <p className="text-sm text-gray-600 mt-2">1080 {t('stars').toLowerCase()}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/95 backdrop-blur-md shadow-xl hover:shadow-2xl transition-shadow">
            ...
            <CardContent>
              <p className="text-5xl font-bold text-gray-800">{worldsCompleted}</p>
              <p className="text-sm text-gray-600 mt-2">12 {t('world').toLowerCase()}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/95 backdrop-blur-md shadow-xl hover:shadow-2xl transition-shadow">
            ...
            <CardContent>
              <p className="text-5xl font-bold text-gray-800">{totalLevelsCompleted}</p>
              <p className="text-sm text-gray-600 mt-2">360 {t('level').toLowerCase()}</p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/95 backdrop-blur-md shadow-xl hover:shadow-2xl transition-shadow">
            ...
            <CardContent>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-800">{THEME_NAMES[currentTheme]}</p>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-gradient-to-br from-red-50 to-red-100 border-2 border-red-200">
                ...
                <span className="text-3xl font-bold text-red-700">{powerUpCounts.bomb}</span>
                <span className="text-sm font-semibold text-red-600">Bomba</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-gradient-to-br from-blue-50 to-blue-100 border-2 border-blue-200">
                ...
                <span className="text-3xl font-bold text-blue-700">{powerUpCounts.clock}</span>
                <span className="text-sm font-semibold text-blue-600">Zaman</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-gradient-to-br from-purple-50 to-purple-100 border-2 border-purple-200">
                ...
                <span className="text-3xl font-bold text-purple-700">{powerUpCounts.shuffle}</span>
                <span className="text-sm font-semibold text-purple-600">Karıştır</span>
              </div>
              <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-gradient-to-br from-yellow-50 to-yellow-100 border-2 border-yellow-200">
                ...
                <span className="text-3xl font-bold text-yellow-700">{powerUpCounts.magnifier}</span>
                <span className="text-sm font-semibold text-yellow-600">İpucu</span>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {playerData.worlds.map((world, index) => {
                ...
                const completedLevels = world.levels.filter(l => l.completed).length;
                const worldStars = world.levels.reduce((sum, l) => sum + l.stars, 0);
                const progressPercent = (completedLevels / 30) * 100;

                return (
                  <div key={world.worldId} className="p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3">
                        <span className="text-3xl">{worldEmoji}</span>
                        <div>
                          <h3 className="font-bold text-lg text-gray-800">{worldName}</h3>
                          <p className="text-sm text-gray-600">
                            {completedLevels}/30 {t('level').toLowerCase()} • {worldStars} {t('stars').toLowerCase()}
                          </p>
                        </div>
                      </div>
                      {!world.unlocked && (
