...
function AppContent() {
  ...
  const [soundManagerInitialized, setSoundManagerInitialized] =
  ...
  useEffect(() => {
    const initSoundManager = async () => {
      ...
      try {
        ...
        console.error(
          "[App] ✗ Failed to initialize native sound manager:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    if (playerData) {
      ...
      console.log(
        "[App] ✓ Found existing player data, loading saved game state",
      );
      ...
      // Load saved language
      const langValue =
        playerData.language as keyof typeof LocalStorage.Language;
      if (LocalStorage.Language[langValue]) {
        setContextLanguage(LocalStorage.Language[langValue]);
      }

      console.log(
        "[App] ✓ Game state restored successfully for user:",
        playerData.username,
      );
    } else {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: re-apply theme on screen change is intentional
  useEffect(() => {
    applyThemeToDocument(currentTheme);
  }, [currentScreen, currentTheme]);
  ...
  return (
    <div className="min-h-screen w-full overflow-hidden">
      ...
      {currentScreen === "admin" && <AdminPanel onBack={handleBackFromAdmin} />}
