...
function getWorldDifficultyMultiplier(worldId: number): number {
  switch (worldId) {
    case 1:
      return 1.0; // Garden - baseline
    case 2:
      return 1.15; // Ocean - 15% harder
    case 3:
      return 1.3; // Candyland - 30% harder
    case 4:
      return 1.45; // Forest - 45% harder
    case 5:
      return 1.6; // Volcano - 60% harder
    case 6:
      return 1.75; // Space - 75% harder
    case 7:
      return 1.9; // Desert - 90% harder
    case 8:
      return 2.05; // Arctic - 105% harder
    case 9:
      return 2.2; // Jungle - 120% harder
    case 10:
      return 2.35; // Crystal - 135% harder
    case 11:
      return 2.5; // Shadow - 150% harder
    case 12:
      return 2.65; // Rainbow - 165% harder
    default:
      return 1.0;
  }
}
...
function calculateAdvancedDifficultyParams(worldId: number, levelId: number) {
  ...
  const pairCount = Math.floor(
    (basePairMin + normalizedLevel * (basePairMax - basePairMin)) *
      bossMultiplier,
  );
  ...
  const baseMinTime = Math.max(
    60,
    120 - Math.floor(30 * (worldMultiplier - 1.0)),
  );
  const timeLimit = Math.floor(
    (baseMaxTime - normalizedLevel * (baseMaxTime - baseMinTime)) /
      (isBoss ? 1.2 : 1.0),
  );
  ...
  const obstacleDensity =
    (baseMinDensity + normalizedLevel * (baseMaxDensity - baseMinDensity)) *
    bossMultiplier;
  ...
  const tileVariety = Math.min(
    FRUIT_TYPES.length,
    4 + Math.floor(worldId * 0.4) + Math.floor(normalizedLevel * 2),
  );
...
function generateBossObjective(worldId: number): BossObjective {
  const objectives: Array<BossObjective["type"]> = [
    "chained",
    "timed",
    "explosion-chain",
  ];
...
function applyWorldMechanics(
  ...
  switch (worldId) {
    ...
    case 3: {
      // Candyland - sweet-themed challenges with candy-specific patterns
    ...
    case 4: {
      // Forest - nature-based obstacles with tree-layer complexity
    ...
    case 5: {
      // Volcano - intense heat-themed challenges with lava-rock obstacles
      const volcanoCoverCount = Math.floor(tileCount * adjustedDensity * 0.75);
      for (let i = 0; i < volcanoCoverCount; i++) {
        tiles[i].covered = true;
        const coverIndex = Math.min(
          i + Math.floor(tileCount / (layerDepth + 1)),
          tileCount - 1,
        );
        tiles[i].coveredBy = tiles[coverIndex].id;
      }
    ...
    case 6: {
      // Space - advanced cosmic puzzles with stellar complexity
    ...
    case 7: {
      // Desert - sandy obstacles with heat waves
    ...
    case 8: {
      // Arctic - ice-themed with frozen obstacles
    ...
    case 9: {
      // Jungle - dense vegetation with complex layers
    ...
    case 10: {
      // Crystal - prismatic challenges with gem-like obstacles
    ...
    case 11: {
      // Shadow - mysterious dark obstacles with maximum complexity
    ...
    case 12: {
      // Rainbow - legendary difficulty with all obstacle types
 ...
 */
export function generateLevel(worldIdIn: number, levelIdIn: number): Level {
  // Validate inputs
  let worldId = worldIdIn;
  let levelId = levelIdIn;
  ...
  const tiles: Tile[] = [];
  const fruitPool = FRUIT_TYPES.slice(
    0,
    Math.min(tileVariety, FRUIT_TYPES.length),
  );
...
export function calculateStarRating(
  ...
  if (
    timePercentage >= threeStarTimeThreshold &&
    moveEfficiency >= threeStarMoveThreshold
  ) {
    return 3;
  }
  ...
  if (
    timePercentage >= twoStarTimeThreshold &&
    moveEfficiency >= twoStarMoveThreshold
  ) {
    return 2;
  }
