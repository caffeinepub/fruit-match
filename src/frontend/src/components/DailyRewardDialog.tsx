...
import { Bomb, Clock, Gift, Lightbulb, Shuffle } from "lucide-react";
...
export default function DailyRewardDialog({
  ...
  const nextDay = streakInfo.isAvailable
    ? streakInfo.currentStreak === 0
      ? 1
      : Math.min(streakInfo.currentStreak + 1, 7)
        ...
        <div className="flex flex-col gap-4 sm:gap-6 py-3 sm:py-4">
          ...
          <div className="flex justify-center gap-1 sm:gap-2 flex-wrap">
            {REWARD_CONFIG.map((reward, index) => {
              ...
              const _isLocked = dayNumber > nextDay;
                ...
                <div className="flex flex-col gap-2 items-center">
                  {currentReward.powerUps.length > 0 && (
                    <div className="flex gap-2 sm:gap-3 mt-2 flex-wrap justify-center">
                      {currentReward.powerUps.map((powerUp) => {
                        const powerUpConfig =
                          POWER_UP_TYPES[
                            powerUp.type as keyof typeof POWER_UP_TYPES
                          ];
                        ...
                        return (
                          <div
                            key={powerUp.type}
                            className="flex items-center gap-1 bg-white/80 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg"
                          >
          ...
          <div className="text-center text-xs sm:text-sm text-muted-foreground px-2">
            {streakInfo.currentStreak > 0 && (
              <p>
                {t("current_streak")}:{" "}
                <span className="font-bold text-fruit-star">
                  {streakInfo.currentStreak}
                </span>{" "}
                {t("days")}
              </p>
            ...
            {nextDay === 7 && streakInfo.isAvailable && (
              <p className="text-fruit-star font-semibold mt-2">
                {t("grand_reward_available")}
              </p>
            )}
          </div>
        </div>
